...
class TestVitalsUUID:
    def test_no_index(self, df):
        """UUID as a column"""
        _df = likeness_vitals.vitals.create_uid(df, "uuid")
        known = numpy.array(
    ...
    def test_with_index(self, df):
        """UUID as the index"""
        _df = likeness_vitals.vitals.create_uid(df, "uuid", set_index=True)
        known = numpy.array(
            [["A", "x", 10], ["B", "x", 20], ["C", "y", 30]], dtype=object
        )
        observed = _df.values
...
class TestVitalsNonUUID:
    def test_1col(self, df):
        ...
        _df = (
            likeness_vitals.vitals.create_uid(df, "id1", from_columns="c1")
            .reset_index()
            .values
        )
        known = numpy.array(
    ...
    def test_2col(self, df):
        ...
        _df = (
            likeness_vitals.vitals.create_uid(df, "id1", from_columns=["c1", "c2"])
            .reset_index()
            .values
        )
        known = numpy.array(
    ...
    def test_1col_idx(self, df):
        ...
        _df = (
            likeness_vitals.vitals.create_uid(
                df, "id1", use_index=True, from_columns="c1"
            )
            .reset_index()
            .values
        )
